...
class IQADatalist():
    ...
    def load_data_dict(self):
        ...
        refpath = os.path.join(self.db_path, 'reference_images')
        refname = getTIDFileName(refpath, '.bmp.BMP')
        txtpath = os.path.join(self.db_path, 'mos_with_names.txt')
        fh = open(txtpath, 'r')
        imgnames = []
        target = []
        refnames_all = []
        ...
        for i, item in enumerate(index):
            ...
            for j, item in enumerate(train_sel):
                d_img_list.append(os.path.join(self.db_path, 'distorted_images', imgnames[item]))
