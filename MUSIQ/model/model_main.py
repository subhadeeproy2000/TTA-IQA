...
class IQARegression(nn.Module):
    ...
    def forward(self, mask_inputs, feat_dis_org, feat_dis_scale_1, feat_dis_scale_2,feat=False):
        # batch x (C=2048) x H x W -> batch x (C=384) x H x W
        feat_dis_org_embed = self.conv_enc(feat_dis_org)
        # print(feat_dis_org.shape)
        # print(self.conv_enc)
        feat_dis_scale_1_embed = self.conv_enc(feat_dis_scale_1)
        # print(feat_dis_scale_1_embed.shape)
        ...
        enc_outputs = self.transformer(mask_inputs, feat_dis_org_embed, feat_dis_scale_1_embed, feat_dis_scale_2_embed,feat)
...
class Transformer(nn.Module):
    ...
    def forward(self, mask_inputs, feat_dis_org_embed, feat_dis_scale_1_embed, feat_dis_scale_2_embed,single=False):
        ...
        enc_outputs, enc_self_attn_probs = self.encoder(mask_inputs, feat_dis_org_embed, feat_dis_scale_1_embed, feat_dis_scale_2_embed,single)
...
class Encoder(nn.Module):
    ...
    def forward(self, mask_inputs, feat_dis_org_embed, feat_dis_scale_1_embed, feat_dis_scale_2_embed,single=False):
        ...
        scale_org_embed = repeat(self.scale_org_embedding, '() c () () -> b c h w', b=self.config.batch_size, h=7, w=7)

        if not single:
            scale_1_embed = repeat(self.scale_1_embedding, '() c () () -> b c h w', b=self.config.batch_size, h=12, w=12)
