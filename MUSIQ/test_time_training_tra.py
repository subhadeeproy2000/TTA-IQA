...
def adapt(model, data_dict, config, optimizer,old_net, model_transformer,mask_inputs):
		...
		f_high['d_img_org'] = []
		f_high['d_img_scale_1'] = []
		f_high['d_img_scale_2'] = []

		old_net.eval()

		with torch.no_grad():
			pred0 = predict(old_net, model_transformer, data_dict['image']['d_img_org'].cuda(),
							data_dict['image']['d_img_scale_1'].cuda(), data_dict['image']['d_img_scale_2'].cuda(),
							mask_inputs)
			...
			if config.rank:
				...
				data_dict['blur_low']['d_img_org'] = T.GaussianBlur(kernel_size=(5, 5), sigma=(sigma2))(
					inputs['d_img_org']).cuda()
				data_dict['blur_high']['d_img_org'] = T.GaussianBlur(kernel_size=(5, 5), sigma=(sigma1))(
					inputs['d_img_org']).cuda()

				data_dict['blur_low']['d_img_scale_1'] = T.GaussianBlur(kernel_size=(5, 5), sigma=(sigma2))(
					inputs['d_img_scale_1']).cuda()
				data_dict['blur_high']['d_img_scale_1'] = T.GaussianBlur(kernel_size=(5, 5), sigma=(sigma1))(
					inputs['d_img_scale_1']).cuda()

				data_dict['blur_low']['d_img_scale_2'] = T.GaussianBlur(kernel_size=(5, 5), sigma=(sigma2))(
					inputs['d_img_scale_2']).cuda()
				data_dict['blur_high']['d_img_scale_2'] = T.GaussianBlur(kernel_size=(5, 5), sigma=(sigma1))(
					inputs['d_img_scale_2']).cuda()
				...
				pred1 = predict(old_net, model_transformer, data_dict['comp_high']['d_img_org'].cuda(),
								data_dict['comp_high']['d_img_scale_1'].cuda(),
								data_dict['comp_high']['d_img_scale_2'].cuda(), mask_inputs)
				pred2 = predict(old_net, model_transformer, data_dict['comp_low']['d_img_org'].cuda(),
								data_dict['comp_low']['d_img_scale_1'].cuda(),
								data_dict['comp_low']['d_img_scale_2'].cuda(), mask_inputs)
				comp = torch.abs(pred0 - pred1) - torch.abs(pred0 - pred2)

				pred3 = predict(old_net, model_transformer, data_dict['nos_high']['d_img_org'].cuda(),
								data_dict['nos_high']['d_img_scale_1'].cuda(),
								data_dict['nos_high']['d_img_scale_2'].cuda(), mask_inputs)
				pred4 = predict(old_net, model_transformer, data_dict['nos_low']['d_img_org'].cuda(),
								data_dict['nos_low']['d_img_scale_1'].cuda(),
								data_dict['nos_low']['d_img_scale_2'].cuda(),
								mask_inputs)
				nos = torch.abs(pred0 - pred3) - torch.abs(pred0 - pred4)

				pred5 = predict(old_net, model_transformer, data_dict['blur_high']['d_img_org'].cuda(),
								data_dict['image']['d_img_scale_1'].cuda(), data_dict['image']['d_img_scale_2'].cuda(),
								mask_inputs)

				pred6 = predict(old_net, model_transformer, data_dict['blur_low']['d_img_org'].cuda(),
								data_dict['image']['d_img_scale_1'].cuda(), data_dict['image']['d_img_scale_2'].cuda(),
								mask_inputs)
				blur = torch.abs(pred0 - pred5) - torch.abs(pred0 - pred6)

				all_diff = torch.cat([comp, nos, blur], dim=1)

				for p in range(len(pred0)):
					if all_diff[p].argmax().item() == 0:
						f_low['d_img_org'].append(id_dict[1]['d_img_org'][p].cuda())
						f_high['d_img_org'].append(id_dict[0]['d_img_org'][p].cuda())
						f_low['d_img_scale_1'].append(id_dict[1]['d_img_scale_1'][p].cuda())
						f_high['d_img_scale_1'].append(id_dict[0]['d_img_scale_1'][p].cuda())
						f_low['d_img_scale_2'].append(id_dict[1]['d_img_scale_2'][p].cuda())
						f_high['d_img_scale_2'].append(id_dict[0]['d_img_scale_2'][p].cuda())
					if all_diff[p].argmax().item() == 1:
						f_low['d_img_org'].append(id_dict[3]['d_img_org'][p].cuda())
						f_high['d_img_org'].append(id_dict[2]['d_img_org'][p].cuda())
						f_low['d_img_scale_1'].append(id_dict[3]['d_img_scale_1'][p].cuda())
						f_high['d_img_scale_1'].append(id_dict[2]['d_img_scale_1'][p].cuda())
						f_low['d_img_scale_2'].append(id_dict[3]['d_img_scale_2'][p].cuda())
						f_high['d_img_scale_2'].append(id_dict[2]['d_img_scale_2'][p].cuda())
					if all_diff[p].argmax().item() == 2:
						f_low['d_img_org'].append(id_dict[5]['d_img_org'][p].cuda())
						f_high['d_img_org'].append(id_dict[4]['d_img_org'][p].cuda())
						f_low['d_img_scale_1'].append(id_dict[5]['d_img_scale_1'][p].cuda())
						f_high['d_img_scale_1'].append(id_dict[4]['d_img_scale_1'][p].cuda())
						f_low['d_img_scale_2'].append(id_dict[5]['d_img_scale_2'][p].cuda())
						f_high['d_img_scale_2'].append(id_dict[4]['d_img_scale_2'][p].cuda())

				f_low['d_img_org'] = torch.squeeze(torch.stack(f_low['d_img_org']), dim=1)
				f_high['d_img_org'] = torch.squeeze(torch.stack(f_high['d_img_org']), dim=1)

				f_low['d_img_scale_1'] = torch.squeeze(torch.stack(f_low['d_img_scale_1']), dim=1)
				f_high['d_img_scale_1'] = torch.squeeze(torch.stack(f_high['d_img_scale_1']), dim=1)

				f_low['d_img_scale_2'] = torch.squeeze(torch.stack(f_low['d_img_scale_2']), dim=1)
				f_high['d_img_scale_2'] = torch.squeeze(torch.stack(f_high['d_img_scale_2']), dim=1)
		...
		if config.blur:
			...
			f_low = T.GaussianBlur(kernel_size=(5, 5), sigma=(sigma2))(inputs['d_img_org']).cuda()
			f_high = T.GaussianBlur(kernel_size=(5, 5), sigma=(sigma1))(inputs['d_img_org']).cuda()
		...
		if config.cls:
			model_transformer.transformer.encoder.cls_token.requires_grad = True
		...
		for layer in model_transformer.modules():
			if config.bn:
				if isinstance(layer, nn.BatchNorm2d):
					layer.requires_grad_(True)
			if config.ln:
		...
		loss_min=99999
		head = head_on_layer2()
		ssh = ExtractorHead(model, head,model_transformer,mask_inputs).cuda()

		ssh.train()

		for iteration in range(config.niter):

			target = torch.ones(inputs['d_img_org'].shape[0]).cuda()
			...
			if config.rank or config.blur or config.comp or config.nos:
				...
				dist_high = torch.nn.PairwiseDistance(p=2)(f_neg_feat, f_actual)
				dist_low = torch.nn.PairwiseDistance(p=2)(f_pos_feat, f_actual)
